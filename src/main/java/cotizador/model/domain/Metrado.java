...
@NamedQueries({
	...
	@NamedQuery(name = "Metrado.allMetradoByProject", query = "SELECT m FROM Metrado m WHERE m.nivel.proyecto.id = :idProyecto order by m.precio.producto.sistema asc"),
	@NamedQuery(name = "Metrado.findNivelesByIdProject", query = "SELECT distinct m.nivel FROM Metrado m WHERE m.nivel.proyecto.id = :idProyecto")
